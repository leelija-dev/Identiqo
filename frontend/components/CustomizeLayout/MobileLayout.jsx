// app/customize/components/MobileLayout.jsx
'use client';
    ...
    <div className="lg:hidden flex flex-col h-full max-h-full overflow-hidden relative">
      ...
      <div className={`flex-shrink-0 bg-gradient-to-br from-indigo-50/30 via-white to-purple-50/30 flex items-start sm:items-center justify-center p-3 relative overflow-y-auto overscroll-contain ${currentOrientation === 'portrait' ? 'h-[55vh] min-h-[400px]' : 'h-[42vh] min-h-[260px] sm:min-h-[300px]'}`}>
        <div className="absolute top-2 right-2 flex gap-2 z-30">
          <button onClick={onBack} className="min-w-[44px] min-h-[44px] w-8 h-8 bg-white rounded-full shadow-lg flex items-center justify-center">
            <FiArrowLeft size={14} />
          </button>
          <div className="relative" ref={mobileDownloadRef}>
            <button onClick={() => setShowMobileDownloadMenu(prev => !prev)} className="min-w-[44px] min-h-[44px] w-8 h-8 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-full shadow-lg flex items-center justify-center">
              <FiDownload size={14} />
            </button>
            {showMobileDownloadMenu && (
          ...
          <button onClick={onFlip} className="min-w-[44px] min-h-[44px] w-8 h-8 bg-white rounded-full shadow-lg text-indigo-600 flex items-center justify-center">
