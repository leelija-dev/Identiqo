  ...
  const downloadTemplate = async () => {
    ...
    try {
      ...
      if (front && back) {
        ...
        showToastMessage('Template downloaded (front + back as single file)');
      } else {
    ...
    <div className="min-h-screen bg-[#f5f7fb] font-['Inter'] overflow-x-hidden">
      ...
      <div className="md:hidden fixed top-20 left-4 z-50">
        <button
          ...
          className="bg-indigo-500 text-white p-3 rounded-full shadow-lg hover:bg-indigo-600 transition-colors"
        >
          <FiMenu size={20} />
        </button>
      </div>
      ...
      <div className="flex min-h-[calc(100vh-70px)]">
        ...
        <aside 
          className={`fixed md:relative top-0 left-0 h-full w-[280px] max-w-[85vw] bg-white border-r border-slate-200 py-6 overflow-y-auto z-40 transition-transform duration-300 ease-in-out md:translate-x-0 ${
              ...
              {filteredTemplates.map((template) => (
                ...
                >
                  <CardPreview
                    ...
                    className="w-full transition-all duration-300 hover:shadow-2xl hover:shadow-black/10"
                  />
                </div>
              ))}
            </div>
      ...
      {showModal && selectedTemplate && (
        <div 
          className="fixed inset-0 bg-black/85 backdrop-blur-md flex items-center justify-center z-[1000] p-4"
          ...
          <div className="flex flex-col md:flex-row gap-6 items-center justify-center w-full max-w-[90vw]">
            ...
            <div className="flex flex-row md:flex-col gap-3 w-full md:w-auto justify-center">
              <button 
                ...
                className="px-5 py-2.5 md:px-6 md:py-3.5 rounded-full font-semibold transition-all text-sm md:text-base bg-white text-red-500 border-2 border-red-500 hover:bg-red-500 hover:text-white"
              >
                ⭐ Wishlist
              </button>
              <button 
                onClick={goToCustomize}
                className="px-5 py-2.5 md:px-6 md:py-3.5 rounded-full font-semibold transition-all text-sm md:text-base bg-gradient-to-br from-indigo-600 to-indigo-500 text-white hover:shadow-lg"
              >
                ✏️ Customize
              </button>
              <button
                onClick={downloadTemplate}
                className="px-5 py-2.5 md:px-6 md:py-3.5 rounded-full font-semibold transition-all text-sm md:text-base bg-emerald-500 text-white hover:bg-emerald-600 hover:shadow-lg flex items-center justify-center gap-2"
              >
                <FiDownload /> Download
              </button>
              <button 
                onClick={closeModal}
                className="px-5 py-2.5 md:px-6 md:py-3.5 rounded-full font-semibold transition-all text-sm md:text-base bg-slate-100 text-slate-500 hover:bg-slate-200"
              >
                ✕ Close
              </button>
            </div>
      ...
      {/* Toast */}
      <div className={`fixed bottom-4 right-4 bg-green-500 text-white px-4 py-2 rounded-full font-semibold transition-all duration-300 z-[1100] text-sm
      ...
      <style jsx global>{`
        ...
        @media (max-width: 320px) {
          .grid { gap: 1rem; }
          button { font-size: 0.75rem; padding: 0.5rem 1rem; }
        }
        @media (max-width: 480px) and (orientation: portrait) {
          .rounded-full { min-height: 44px; }
        }
      `}</style>
    </div>
  );
